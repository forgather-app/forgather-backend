...
public record UpdateHostProfileRequest(
    ...
    @Valid
    @Schema(description = "새로 업로드한 프로필 사진. 전달하면 기존 사진을 교체한다.", nullable = true)
    RegisterHostProfilePhotoRequest photo,

    @Schema(description = "프로필 사진 삭제 여부. photo 없이 true면 사진을 제거한다.", example = "true", nullable = true)
