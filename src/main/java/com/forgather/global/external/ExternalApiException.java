 ...
 * 응답 status와 로그 레벨은 {@link FailureType}에서 파생되므로 호출부가 정책을 결정하지 않는다.
...
public class ExternalApiException extends BaseException {
    ...
    /**
     * SimpleClientHttpRequestFactory(HttpURLConnection)는 connect/read를 모두 SocketTimeoutException으로 내므로
     * 메시지로 구분한다. 전송 계층을 JDK HttpClient로 바꾸면 타입만으로 갈린다.
     */
