...
description: 구현이나 수정이 끝난 뒤 현재 작업트리의 변경사항을 검토하고, 사용자 변경사항을 보존한 채 요청된 변경 범위만 작업 단위별 한국어 커밋으로 나누며, 실제 커밋 전 사용자에게 커밋 구분안을 보여주고 승인을 받는다. 사용자가 "커밋해줘", "작업 단위로 커밋", "변경사항 커밋", "구현 변경사항만 커밋", "$commit-work-scope"를 요청하면 사용한다.
---

# 작업 범위 커밋

구현 완료 후 변경사항을 섞지 않고 작업 단위별로 커밋한다. 핵심은 **범위 확인**, **사용자 변경 보존**, **커밋 구분안 승인**, **명시적 path 커밋**이다.
...
- 작업 단위를 나눈 뒤에는 `git add` 또는 `git commit`을 실행하기 전에 사용자에게 커밋 구분안을 보여주고 승인을 받는다.
- 사용자가 커밋 구분을 수정하라고 하면 구분안을 다시 작성해 재승인을 받는다.
...
작업 단위를 나눈 뒤 실제 스테이징과 커밋 전에 사용자에게 구분안을 보여준다.
...
사용자가 승인하기 전에는 새로 `git add` 또는 `git commit`을 실행하지 않는다. 이미 staged 된 무관한 파일이 있어도 임의로 되돌리지 말고, 승인 후 커밋할 때 `git commit --only -- <paths>`를 사용한다.
